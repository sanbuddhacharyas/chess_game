...
class CSquare 
{
    public: 
        ...
        int     FromString(const char *s)  //IT converts the data input by the users in format to vector for board
        {
            uint32_t col = s[0] - 'a';//reduction character 'a' from first string
            uint32_t row = s[1] - '1';//reduction 1 from  2nd string
            m_sq = row*10 + col + 21; //multiplying the combination
            return (col >= 8 || row >= 8);
